...
class MultiStepAgent:
    ...
    def __init__(
        self,
        tools: Dict[str, Tool],
        model: Union[Callable, Any],  # More flexible type annotation to support CrewAI LLM
...
def _is_crewai_llm(model: Any) -> bool:
    """Check if the model is a CrewAI LLM instance."""
    return hasattr(model, 'call') and hasattr(model, 'model') and hasattr(model, 'api_key')


def _convert_messages_to_prompt(messages: List[Dict[str, Any]], tools: List[Tool]) -> str:
    """Convert structured messages to a prompt format for CrewAI LLM."""
    prompt_parts = []
    ...
    # Add tool information to the prompt if available
    if tools:
        prompt_parts.append("\nAvailable tools:")
        for tool in tools:
            if hasattr(tool, 'name') and hasattr(tool, 'description'):
                ...
                if hasattr(tool, 'inputs') and isinstance(tool.inputs, dict):
                    inputs_desc = ", ".join([f"{name}: {info.get('description', name) if isinstance(info, dict) else str(info)}" 
                                           for name, info in tool.inputs.items()])
                    prompt_parts.append(f"  Inputs: {inputs_desc}")
        
        prompt_parts.append("\nTo use a tool, respond in this exact format:")
        prompt_parts.append("TOOL_CALL: tool_name")
        prompt_parts.append("ARGUMENTS: {\"key\": \"value\"}")
        prompt_parts.append("\nOr if you have a final answer, use:")
        prompt_parts.append("TOOL_CALL: final_answer")
        prompt_parts.append("ARGUMENTS: {\"answer\": \"your final answer\"}")
    
    return "\n".join(prompt_parts)


def _parse_crewai_response(response_text: str) -> ChatMessage:
    """Parse CrewAI LLM response and extract tool calls."""
    ...
    if "TOOL_CALL:" in content:
        ...
        for line in lines:
            ...
            if line.startswith("TOOL_CALL:"):
                tool_name = line.split("TOOL_CALL:", 1)[1].strip()
            elif line.startswith("ARGUMENTS:") and tool_name:
                try:
                    args_text = line.split("ARGUMENTS:", 1)[1].strip()
                    arguments = json.loads(args_text)
                except json.JSONDecodeError:
                    # If JSON parsing fails, use the text as input
...
class ACPCallingAgent(MultiStepAgent):
    ...
    Args:
        ...
        model: Model that will generate the agent's actions. Can be a callable function or CrewAI LLM instance.
        prompt_templates ([`Dict[str, str]`], *optional*): Prompt templates.
        planning_interval (`int`, *optional*): Interval at which the agent will run a planning step.
        **kwargs: Additional keyword arguments.
    """
    def __init__(self, acp_agents: Dict[str, Agent], model: Union[Callable, Any],
                 prompt_templates: Optional[Dict[str,str]]=None, planning_interval:Optional[int]=None, **kwargs):
        ...
        # Convert acp agents to tool format
        acp_tools={}
        for name, agent in acp_agents.items():
            # Create a callable tool that will invoke acp agent
            acp_tools[name]=Tool(
                name=name,
            ...
            # Override the __call__ method in Tool to make it actually call the ACP agent
    ...
    def initialize_system_prompt(self) -> str:
        ...
        system_prompt=populate_template(self.prompt_templates["system_prompt"], variables={"agents": agent_descriptions})
    ...
    async def step(self, memory_step: ActionStep) -> Union[None, Any]:
        ...
        if _is_crewai_llm(self.model):
            # CrewAI LLM - convert messages to prompt format
            tools_list = list(self.tools.values())[:-1]  # Exclude final_answer tool from display
            prompt = _convert_messages_to_prompt(memory_messages, tools_list)
            ...
            try:
                ...
                model_message = _parse_crewai_response(response_text)
                memory_step.model_output_message = model_message
                
            except Exception as e:
                raise AgentParsingError(f"Error while generating or parsing CrewAI output:\n{e}", self.logger) from e
        else:
            # Traditional callable model (e.g., smolagents)
            # Make sure all messages are in LiteLLM format
        ...
        if not hasattr(model_message, 'tool_calls') or model_message.tool_calls is None or len(model_message.tool_calls)==0:
            # Not tool call -> treat content as final answer
                            ...
                            if len(parts)>1:
                                ...
                                if arg_line:
                                    ...
                                    if len(arg_parts) > 1:
                                        ...
                                        except json.JSONDecodeError: # If not valid JSON, use as string
    ...
    async def _process_tool_call(self, memory_step: ActionStep, agent_name:str, agent_arguments: Any) -> Union[None, Any]:
        ...
        if agent_name=="final_answer":
            ...
            # If answer refers to a state variable, use it
            if isinstance(answer, str) and answer in self.state:
                ...
                final_answer=answer
                self.logger.log(f"Final answer: {final_answer}, level=LogLevel.INFO")
            memory_step.action_output = final_answer
            return final_answer
        
        else: # Execute ACP agent call
            if agent_arguments is None:
                agent_arguments={}

            observation=await self.execute_tool_call(agent_name, agent_arguments)
            updated_information=str(observation).strip()
            # save to agent's persistent memory, i.e. in a state-variable dictionary
            self.save_to_memory(f"{agent_name}_reponse", updated_information)
            self.logger.log(
